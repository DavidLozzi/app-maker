...
from app.prompts.run_tests import initial_assessment_prompt, debug_prompt
...
from app.utils.file_system import empty_directory, get_folder_inventory
...
def doit():
    ...
    folder_struct = get_folder_inventory(args.path)
    readme = get_file_contents(args.path, ["README.md"])
    prefix = f"""FOLDERS:
{folder_struct}
...
- The testing environment is set up and ready to go."""
    ...
    if args.mode == "tests":
        ...
        while True:
            ...
            if not actions:
                break
            task_output = run_tools(actions, args.path)
            # HANDLE clear_previous action
            previous += f"{task_output}\n-----\n"
            log.info(previous)
            messages = [
                system_message(debug_prompt()),
                user_message(f"{prefix}\n\n{previous}"),
            ]
            log.info("Running prompt debug_prompt")
            response = gpt_messages(messages=messages, in_json=True)
    ...
    {response}\n\n"""
        while current_step <= last_step:
            for task in [
                task for task in action_plans if task["order"] == current_step
            ]:
                ...
                task_output = run_tools(
                    task["action_plan_details"]["actions"], args.path
                )
